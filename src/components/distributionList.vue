			...
			<div class="list">
				<vue-scroll>
					<div v-for="(item) in list"
						...
						<div class="amout">{{item.paymentAmount}}</div>
					</div>
				</vue-scroll>
			</div>
			<div @click="closeMask"
...
<script>
import { queryInvestPaymentPlan } from '@/service/coreApi';
export default {
  props: ['id'],
  data () {
    return {
  ...
  created () {
    this.queryInvestPaymentPlan();
  },
  methods: {
    closeMask () {
      this.$emit('closeMask');
    },
    async queryInvestPaymentPlan () {
      const params = {
        id: this.id
      };
      const res = await queryInvestPaymentPlan(params);
